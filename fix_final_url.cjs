...
const regex = /let logoSrc = logoUrl \|\|[^]+?if \(logoAttachment\) {\r?\n\s*emailPayload\.attachments\.push\(logoAttachment\);\r?\n\s*}/m;

const replacement = `
          let definitiveLogoUrl = logoUrl || '';
          if (!definitiveLogoUrl && logoBase64?.startsWith('http')) {
            definitiveLogoUrl = logoBase64;
          }
          
          if (definitiveLogoUrl && !definitiveLogoUrl.startsWith('http') && !definitiveLogoUrl.startsWith('data:')) {
            // It's a relative Supabase path. We construct the full URL.
            const baseUrl = import.meta.env.VITE_SUPABASE_URL || '';
            if (baseUrl) {
               let cleanPath = definitiveLogoUrl;
               ...
               definitiveLogoUrl = ''; // Fallback to base64 or hide if we cant resolve
            }
          }
          ...
          } else if (logoBase64 && logoBase64.length > 100) {
             // It's raw base64. Wrap as inline attachment for standard email handling
             logoHtml = \`<div style="text-align: center; margin-bottom: 30px;">
