...
-- Total Tables: 70
-- Cloud SQL Compatible: Yes
-- Supabase Dependencies: Removed
-- ============================================
--
-- CLOUD SQL MIGRATION NOTES:
-- ============================================
--
-- This schema has been adapted from Supabase PostgreSQL to Google Cloud SQL.
-- Key changes made:
--
-- 1. EXTENSIONS:
--    - Removed: pg_graphql (Supabase-specific)
--    - Removed: supabase_vault (Supabase-specific)
--    - Kept: uuid-ossp, pgcrypto, pg_stat_statements (standard PostgreSQL)
--    - Changed: Removed "WITH SCHEMA extensions" - using default public schema
--
-- 2. AUTHENTICATION:
--    - All references to auth.users table have been commented out
--    - All references to auth.uid() have been commented out
--    - You will need to implement your own authentication system
--    - Consider using: Cloud Identity Platform, Firebase Auth, or custom auth table
--
-- 3. ROW LEVEL SECURITY (RLS):
--    - RLS policies are preserved but may need modification
--    - Policies reference custom session functions (get_session_organization_id, etc.)
--    - These functions use current_setting() which is standard PostgreSQL
--    - You'll need to set session variables in your application:
--      SET LOCAL app.organization_id = 'uuid-here';
--      SET LOCAL app.environment_id = 'uuid-here';
--
-- 4. FOREIGN KEY CONSTRAINTS:
--    - Foreign keys to auth.users have been commented out
--    - You may want to create a users authentication table or use application-level validation
--
-- ============================================
-- CLOUD SQL RECOMMENDED CONFIGURATION:
-- ============================================
--
-- Recommended Cloud SQL flags for optimal performance:
--
-- Connection & Memory:
--   max_connections = 100-500 (based on your tier)
...
--
-- Connection Pooling:
--   Consider using Cloud SQL Proxy or PgBouncer for connection pooling
--   PgBouncer recommended for applications with many short-lived connections
--
-- Monitoring:
...
--
-- Apply flags via: gcloud sql instances patch INSTANCE_NAME --database-flags=...
--
-- ============================================
-- SCHEMA OVERVIEW:
-- ============================================
...
ALTER TABLE ONLY public.countries
    ...
    -- ADD CONSTRAINT countries_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.states
    ...
    -- ADD CONSTRAINT states_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
    -- ADD CONSTRAINT states_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.carriers
    ...
    -- ADD CONSTRAINT carriers_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
    -- ADD CONSTRAINT carriers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.occurrences
    ...
    -- ADD CONSTRAINT occurrences_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
    -- ADD CONSTRAINT occurrences_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.rejection_reasons
    ...
    -- ADD CONSTRAINT rejection_reasons_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
    -- ADD CONSTRAINT rejection_reasons_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.invoices_nfe
    ...
    -- ADD CONSTRAINT invoices_nfe_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
    -- ADD CONSTRAINT invoices_nfe_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
ALTER TABLE ONLY public.invoices_nfe_occurrences
    ...
    -- ADD CONSTRAINT invoices_nfe_occurrences_responsible_user_id_fkey FOREIGN KEY (responsible_user_id) REFERENCES auth.users(id) -- REMOVED: Supabase auth.users not available in Cloud SQL
...
BEGIN
    ...
    -- Original implementation checked auth.users table for is_saas_admin metadata
    -- Implement custom authentication logic here based on your auth system
    -- Example: Check session variable or custom auth table
    -- RETURN COALESCE(
