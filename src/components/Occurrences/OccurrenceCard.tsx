    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium mt-1 ${getStatusColor()}`}>
              {isDeliveryProblem ? t('occurrences.card.problem') : t('occurrences.card.delivery')}
            </div>
          </div>
        </div>
          ...
          {isAdmin && (
            <>
              <button 
                ...
                className="text-gray-400 hover:text-gray-600 dark:text-gray-400 p-1 rounded hover:bg-gray-50 dark:bg-gray-900 transition-colors"
                title={t('occurrences.card.edit')}
              >
                <Edit size={16} />
              </button>
              <button 
                onClick={() => onDelete(Number(occurrence.id))}
                className="text-red-400 hover:text-red-600 p-1 rounded hover:bg-red-50 transition-colors"
                title={t('occurrences.card.delete')}
              >
                <Trash2 size={16} />
              </button>
            </>
      ...
      <div className="space-y-3 text-sm">
        <div className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
          <Hash size={14} />
          <span>{t('occurrences.card.code')}: {occurrence.codigo}</span>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <div className="text-sm">
          <span className="text-gray-600 dark:text-gray-400">{t('occurrences.card.type')}:</span>
