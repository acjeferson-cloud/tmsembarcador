    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow flex flex-col h-full">
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center space-x-4">
          <div className="w-16 h-12 flex-shrink-0 flex items-center justify-center rounded-lg overflow-hidden bg-gray-50 border border-gray-100 dark:border-gray-700 dark:bg-gray-900 shadow-sm">
            {country.bandeira_url ? (
              <img src={country.bandeira_url} alt={`${t('countries.form.flagOf', { defaultValue: 'Bandeira de' })} ${country.name}`} className="w-full h-full object-cover" />
            ) : country.flag ? (
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('countries.form.code')} : {country.code}</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <button 
            ...
            title={t('countries.actions.view')}
          >
            <Eye size={16} />
          </button>
          {isAdmin && (
      ...
      <div className="space-y-3 text-sm flex-grow mb-6">
        <div className="flex items-center space-x-3 text-gray-600 dark:text-gray-400">
          ...
          <span>{country.continent || t('countries.form.notInformed', { defaultValue: 'Não informado' })}</span>
        </div>
        
        <div className="flex items-center space-x-3 text-gray-600 dark:text-gray-400">
          <MapPin size={16} className="text-gray-400" />
          <span>{country.capital || t('countries.form.notInformed', { defaultValue: 'Não informado' })}</span>
        </div>
        
        <div className="flex items-center space-x-3 text-gray-600 dark:text-gray-400">
          <Languages size={16} className="text-gray-400" />
          <span>{country.language || t('countries.form.notInformed', { defaultValue: 'Não informado' })}</span>
        </div>
      </div>
      
      <div className="pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
        <div className="text-sm">
          <span className="text-gray-500 dark:text-gray-400">{t('countries.form.code')} : </span>
