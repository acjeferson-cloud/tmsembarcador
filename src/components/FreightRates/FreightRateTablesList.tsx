    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">
            {carrierId ? `Tabelas de Frete - ${carrierName}` : 'Todas as Tabelas de Frete'}
          </h2>
          <p className="text-gray-600 dark:text-gray-400">Gerencie as tabelas de frete e tarifas</p>
        </div>
        <button 
        ...
        {filteredTables.map((table) => (
          <div key={table.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
                <DollarSign size={14} />
                <span>{table.tarifas.length} tarifas cadastradas</span>
              </div>

              <div className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
                <Clock size={14} />
                <span>Criada em: {formatDate(table.created_at)}</span>
              </div>
              ...
              <div className="flex items-center space-x-2 flex-wrap gap-2">
                ...
                `}>
                  {table.table_type === 'Entrada'
                    ? 'Entrada (Frete de Compra)'
                    : 'Saída (Frete de Venda)'}
                </div>
      ...
      {filteredTables.length === 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhuma tabela de frete encontrada</h3>
          <p className="text-gray-600 dark:text-gray-400">
            {carrierId 
              ? 'Este transportador ainda não possui tabelas de frete cadastradas.' 
              : 'Nenhuma tabela de frete corresponde aos filtros aplicados.'}
          </p>
          <button 
            onClick={handleNewTable}
