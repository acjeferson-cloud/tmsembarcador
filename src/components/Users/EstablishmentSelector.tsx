    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 md:items-center">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder={t('users.establishments.searchPlaceholder')}
            value={searchTerm}
        ...
        <div className="flex space-x-2">
          ...
          >
            {t('users.establishments.selectAll')}
          </button>
          <button
            type="button"
            onClick={clearAll}
          ...
          >
            {t('users.establishments.clearAll')}
          </button>
        </div>
      </div>
      ...
      <div className="p-4 bg-blue-50 rounded-lg">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <p className="text-sm text-blue-800 font-medium">{t('users.establishments.title')}</p>
            <p className="text-xs text-blue-700 mt-1">
              {t('users.establishments.desc')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden max-h-96 overflow-y-auto">
        <div className="bg-gray-50 dark:bg-gray-900 px-4 py-3 border-b border-gray-200 dark:border-gray-700 sticky top-0">
          <div className="grid grid-cols-12 gap-4">
            <div className="col-span-8 font-medium text-gray-700 dark:text-gray-300">{t('users.establishments.tableEst')}</div>
            <div className="col-span-4 font-medium text-gray-700 dark:text-gray-300">{t('users.establishments.tableAccess')}</div>
          </div>
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {filteredEstablishments.length > 0 ? (
                  ...
                  <div className="grid grid-cols-12 gap-4 items-center">
                    ...
                    <div className="col-span-4">
                      ...
                      >
                        ...
                        <span className="ml-2">
                          {localEstablishments.includes(establishment.codigo) ? t('users.establishments.allowed') : t('users.establishments.notAllowed')}
                        </span>
                      </button>
                    </div>
                  </div>
            ...
            <div className="px-4 py-6 text-center text-gray-500 dark:text-gray-400">
              {t('users.establishments.notFound')}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-700 dark:text-gray-300">
            {localEstablishments.length} {t('users.establishments.selectedCount')}
          </span>
          <span className="text-sm text-gray-700 dark:text-gray-300">
