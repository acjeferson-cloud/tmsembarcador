...
export const CatalogItemForm: React.FC<CatalogItemFormProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.item_code.trim()) {
      setCodeError(t('catalogItems.codeRequired'));
      return;
    }

    if (!formData.item_description.trim()) {
      setCodeError(t('catalogItems.descRequired'));
      return;
    }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        >
          <ArrowLeft size={20} />
          <span>{t('catalogItems.backToCatalog')}</span>
        </button>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          {item ? t('catalogItems.editItem') : t('catalogItems.newItem')}
        </h1>
        <p className="text-gray-600 dark:text-gray-400">{t('catalogItems.fillData')}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{t('catalogItems.itemInfo')}</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {t('catalogItems.internalCodeLabel')}
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder={t('catalogItems.internalCodePlaceholder')}
                />
              </div>
              ...
              <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div className="flex items-start space-x-2">
                  ...
                  <div>
                    <p className="text-sm text-blue-800 dark:text-blue-300 font-medium">{t('catalogItems.productCodeTitle')}</p>
                    <p className="text-xs text-blue-700 dark:text-blue-400 mt-1">
                      {t('catalogItems.productCodeDesc')}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                placeholder={t('catalogItems.descPlaceholder')}
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {t('catalogItems.maxChars')}{100 - formData.item_description.length}
              </p>
            </div>
            
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {t('catalogItems.eanLabel')}
              </label>
              <div className="relative">
                <input
                  type="text"
                  ...
                  placeholder={t('catalogItems.eanPlaceholder')}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-900 dark:text-white"
                  placeholder={t('catalogItems.ncmPlaceholder')}
                  maxLength={8}
                />
              </div>
            </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-xl p-6">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <h3 className="text-lg font-semibold text-blue-900 dark:text-blue-300 mb-2">{t('catalogItems.aboutRestrictionsTitle')}</h3>
              <p className="text-blue-800 dark:text-blue-200/80 mb-4">
                {t('catalogItems.aboutRestrictionsDesc')}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div className="bg-white dark:bg-gray-800 rounded-lg p-3">
                  <p className="font-semibold text-blue-900 dark:text-blue-300">{t('catalogItems.ncmEanTitle')}</p>
                  <p className="text-blue-700 dark:text-blue-400">{t('catalogItems.ncmEanDesc')}</p>
                </div>
                <div className="bg-white dark:bg-gray-800 rounded-lg p-3">
                  <p className="font-semibold text-blue-900 dark:text-blue-300">{t('catalogItems.bindingTitle')}</p>
                  <p className="text-blue-700 dark:text-blue-400">{t('catalogItems.bindingDesc')}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          >
            {t('catalogItems.cancel')}
          </button>
          <button
            type="submit"
            className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {item ? t('catalogItems.updateBtn') : t('catalogItems.saveBtn')}
          </button>
        </div>
      </form>
    </div>
