...
serve(async (req) => {
  ...
  try {
    ...
    const { startDate, endDate, carrierIds, organizationId, environmentId } = body;
    ...
    const results = carrierIds.map(id => ({
      carrierId: id,
      totalCost: 0,
      averageCost: 0,
      optimizedOrdersCount: 0, // This carrier was the cheapest for this many orders
      validOrdersCount: 0, // Orders where this carrier has > 0 cost
    }));
    ...
    // Load active tables for selected carriers
    for (const carrierId of carrierIds) {
      ...
      if (tableData) {
        freightTableCache.set(carrierId, tableData.id);
      }
    }
    ...
    for (const order of orders) {
      ...
      for (const carrierId of carrierIds) {
        const tableId = freightTableCache.get(carrierId);
        if (!tableId || !cityId) continue;
        ...
        if (tariff) {
          ...
          // Simplificacao: ICMS embutido ignorado nesta amostragem por limitação de tempo,
          // ou aplicando simplificacao rapida.
