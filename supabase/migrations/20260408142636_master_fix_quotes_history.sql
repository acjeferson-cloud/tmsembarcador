...
-- 2. Ensure RLS is totally disabled just in case
ALTER TABLE freight_quotes_history DISABLE ROW LEVEL SECURITY;

-- 3. Replace the bypassing function (creates it)
CREATE OR REPLACE FUNCTION public.bypass_insert_freight_quotes_history(payload JSONB)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 4. Tell PostgREST to flush its cache so it registers the disabled RLS and the new RPC!
