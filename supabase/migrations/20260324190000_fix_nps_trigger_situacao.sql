-- Fix the NPS Queue Trigger to use the correct column name 'situacao' instead of 'status'
-- This resolves the "record 'new' has no field 'status'" error when saving a Delivery Proof on invoices_nfe.
...
DECLARE
    ...
    v_org_id UUID;
BEGIN
    -- Check if situacao changed TO 'ENTREGUE'
    IF NEW.situacao = 'ENTREGUE' AND (OLD.situacao IS NULL OR OLD.situacao != 'ENTREGUE') THEN
