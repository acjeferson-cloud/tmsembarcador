/*
  # Funções RPC para agregação de dados do Dashboard (TMS BI)
  # Expandido com 7 novos KPIs (Representatividade, Custo Médio, Divergências, Lead Time, Backlog, SLA Coleta, Falhas)
*/
...
DECLARE
  ...
  v_volumes_ton numeric;
  v_mercadoria_valor numeric;
  v_total_embarques int;
  v_entregas_no_prazo int;
  v_ticket_medio numeric;
  v_sla_otif numeric;
  v_representatividade numeric;
  v_custo_kg numeric;
  v_divergencia_frete numeric;
  v_taxas_extras_valor numeric;
  v_taxas_extras_percent numeric;
  v_result jsonb;
BEGIN
  ...
  -- Custo Total, Peso, Valor da Mercadoria (CTEs Completos)
  SELECT 
    COALESCE(SUM(total_value), 0),
    COALESCE(SUM(cargo_weight), 0),
    COALESCE(SUM(cargo_value), 0),
    COALESCE(SUM(COALESCE(seccat_value, 0) + COALESCE(dispatch_value, 0) + COALESCE(ademe_gris_value, 0) + COALESCE(itr_value, 0) + COALESCE(tas_value, 0) + COALESCE(collection_delivery_value, 0) + COALESCE(other_tax_value, 0) + COALESCE(toll_value, 0)), 0)
  INTO 
    v_custo_frete, v_volumes_ton, v_mercadoria_valor, v_taxas_extras_valor
  ...
  WHERE (v_org_id IS NULL OR c.organization_id = v_org_id)
    ...
    AND (p_uf IS NULL OR c.recipient_state = p_uf);

  -- Custo Divergência (Da Tabela de Origem CTEs)
  SELECT 
    COALESCE(SUM(valor_divergencia), 0)
  INTO 
    v_divergencia_frete
  FROM ctes c
  WHERE (v_org_id IS NULL OR c.organization_id = v_org_id)
    AND (v_env_id IS NULL OR c.environment_id = v_env_id)
    AND DATE(c.data_emissao) >= p_start_date
    AND DATE(c.data_emissao) <= p_end_date
    AND c.divergencia_valores = true
    AND (p_carrier_id IS NULL OR c.carrier_id = p_carrier_id);

  -- Embarques e Prazo
  SELECT 
    COUNT(1),
    COUNT(CASE WHEN data_entrega_realizada <= data_prevista_entrega THEN 1 END)
  INTO 
    v_total_embarques, v_entregas_no_prazo
  FROM orders o
  WHERE (v_org_id IS NULL OR o.organization_id = v_org_id)
    AND (v_env_id IS NULL OR o.environment_id = v_env_id)
    AND o.data_pedido >= p_start_date
    AND o.data_pedido <= p_end_date
    AND o.status != 'cancelado'
    AND (p_carrier_id IS NULL OR o.carrier_id = p_carrier_id)
    AND (p_uf IS NULL OR o.destino_estado = p_uf);
  ...
  BEGIN
    ...
    WHERE (v_org_id IS NULL OR o.organization_id = v_org_id) AND (v_env_id IS NULL OR o.environment_id = v_env_id)
      AND o.data_pedido >= p_start_date AND o.data_pedido <= p_end_date AND o.status = 'entregue'
      AND (p_carrier_id IS NULL OR o.carrier_id = p_carrier_id) AND (p_uf IS NULL OR o.destino_estado = p_uf);
      
    v_sla_otif := CASE WHEN v_entregues > 0 THEN (v_entregas_no_prazo::numeric / v_entregues::numeric) * 100 ELSE 100 END;
  END;

  v_result := jsonb_build_object(
    'custoTotalFrete', v_custo_frete,
    'volumeToneladas', v_volumes_ton / 1000.0,
    'totalEmbarques', v_total_embarques,
    'ticketMedio', CAST(v_ticket_medio AS numeric(15,2)),
    'slaOtif', CAST(v_sla_otif AS numeric(15,1)),
    'representatividade', CAST(v_representatividade AS numeric(15,2)),
    'custoKg', CAST(v_custo_kg AS numeric(15,2)),
    'custoDivergencia', v_divergencia_frete,
    'taxasExtrasPercent', CAST(v_taxas_extras_percent AS numeric(15,2))
  );
...
DECLARE
  v_org_id uuid; v_env_id uuid; v_result jsonb;
BEGIN
  v_org_id := nullif(current_setting('request.jwt.claim.organization_id', true), '')::uuid;
  v_env_id := nullif(current_setting('request.jwt.claim.environment_id', true), '')::uuid;
  IF v_org_id IS NULL THEN
  ...
  custos_diarios AS (
    ...
    WHERE (v_org_id IS NULL OR organization_id = v_org_id) AND (v_env_id IS NULL OR environment_id = v_env_id)
      AND LOWER(status) in ('autorizado', 'aprovado', 'importado', 'processando')
      AND DATE(issue_date) >= p_start_date AND DATE(issue_date) <= p_end_date
      AND (p_carrier_id IS NULL OR carrier_id = p_carrier_id) AND (p_uf IS NULL OR recipient_state = p_uf)
    GROUP BY DATE(issue_date)
  )
  SELECT jsonb_agg(
    jsonb_build_object('data', to_char(d.data_evolucao, 'DD/MM'), 'custo', COALESCE(c.custo, 0), 'entregas', COALESCE(c.qtd_ctes, 0)) ORDER BY d.data_evolucao
  ) INTO v_result FROM dias d LEFT JOIN custos_diarios c ON d.data_evolucao = c.data_ct;
...
BEGIN
  ...
  WITH ranking AS (
    ...
    WHERE (v_org_id IS NULL OR cte.organization_id = v_org_id) AND (v_env_id IS NULL OR cte.environment_id = v_env_id)
      AND LOWER(cte.status) IN ('autorizado', 'aprovado', 'importado', 'processando')
      AND DATE(cte.issue_date) >= p_start_date AND DATE(cte.issue_date) <= p_end_date
      AND (p_uf IS NULL OR cte.recipient_state = p_uf)
    GROUP BY car.id, car.razao_social ORDER BY valor_frete DESC LIMIT 5
  )
...
BEGIN
  ...
  WITH status_count AS (
    ...
    WHERE (v_org_id IS NULL OR organization_id = v_org_id) AND (v_env_id IS NULL OR environment_id = v_env_id)
      ...
      AND (p_carrier_id IS NULL OR carrier_id = p_carrier_id) AND (p_uf IS NULL OR destino_estado = p_uf)
    GROUP BY status
  )
...
BEGIN
  ...
  -- 4. Top Ocorrências de Problemas de Coleta
  SELECT jsonb_agg(jsonb_build_object('descricao', r.descricao, 'quantidade', cnt)) INTO v_ocorrencias
  FROM (
     SELECT r.descricao, COUNT(p.id) as cnt
     FROM pickups p
     JOIN rejection_reasons r ON r.id = p.rejection_reason_id
     WHERE p.status = 'rejeitada' AND DATE(p.data_agendada) >= p_start_date AND DATE(p.data_agendada) <= p_end_date AND (v_org_id IS NULL OR p.organization_id = v_org_id) AND (v_env_id IS NULL OR p.environment_id = v_env_id) AND (p_carrier_id IS NULL OR p.carrier_id = p_carrier_id)
...
BEGIN
  ...
  WITH cidade_resumo AS (
    SELECT 
      recipient_city as cidade,
      recipient_state as uf,
      SUM(total_value) as custo_total,
      COUNT(1) as total_entregas,
      SUM(cargo_weight) as volume_kg
    FROM ctes_complete c
    WHERE (v_org_id IS NULL OR c.organization_id = v_org_id)
      AND (v_env_id IS NULL OR c.environment_id = v_env_id)
      AND LOWER(c.status) IN ('autorizado', 'aprovado', 'importado', 'processando')
