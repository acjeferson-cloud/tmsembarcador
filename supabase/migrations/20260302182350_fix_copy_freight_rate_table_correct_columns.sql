/*
  # Corrigir função copy_freight_rate_table - usar colunas corretas

  1. Alterações
    - Atualizar cópia de freight_rate_details com colunas reais da tabela
    - Remover referências a valor_kg e valor_adicional (não existem)
    - Usar colunas: peso_ate, m3_ate, volume_ate, valor_ate, valor_faixa, etc
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Copiar detalhes da tarifa (freight_rate_details) com colunas corretas
    FOR v_detail IN
    ...
    LOOP
      ...
      VALUES (
        ...
        v_detail.volume_ate,
        v_detail.valor_ate,
        v_detail.valor_faixa,
        v_detail.tipo_calculo,
        v_detail.tipo_frete,
        v_detail.frete_valor,
        v_detail.frete_minimo,
        v_detail.tipo_taxa,
        v_detail.taxa_minima
      );
    END LOOP;
...
EXCEPTION WHEN OTHERS THEN
  -- Em caso de erro, retornar mensagem detalhado
  RETURN jsonb_build_object(
    'success', false,
    'error', SQLERRM,
    'detail', SQLSTATE
