/*
  # Corrigir função get_user_establishments para aceitar status NULL

  1. Problema
    - Alguns usuários têm status = NULL
    - A função filtrava por status = 'ativo', excluindo usuários com status NULL
  
  2. Solução
    - Aceitar tanto status = 'ativo' quanto status IS NULL
    - Bloquear apenas status = 'bloqueado' ou 'inativo'
*/
...
COMMENT ON FUNCTION get_user_establishments IS
  'Retorna estabelecimentos filtrados por org/env usando email do usuário. Aceita status ativo ou NULL.';
