-- Fix NPS Queue Trigger by extracting the Destination CNPJ/CPF from the `invoices_nfe_customers` table
-- if it is not present in the invoice's `metadata->dest`, which prevents the fallback to XML emails.
