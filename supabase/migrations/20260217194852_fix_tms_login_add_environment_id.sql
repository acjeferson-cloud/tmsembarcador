/*
  # Corrigir função tms_login para retornar environment_id

  ## Problema Crítico
  - A função tms_login não estava retornando environment_id
  - Isso causava que carriersService não conseguisse buscar dados
  - RLS bloqueava porque não tinha environment_id no contexto

  ## Solução
  - Adicionar environment_id ao retorno da função
  - Adicionar codigo do usuário também
...
'Autentica usuário no TMS. Retorna user_id, organization_id, environment_id e outras informações necessárias.';
