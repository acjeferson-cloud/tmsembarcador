...
export const freightCostCalculator = {
    /**
     * Remove acentos e converte string para maiúsculas para comparação
     */
    normalizeString(str) {
        if (!str)
            return '';
     ...
     */
    async findActiveFreightTable(carrierId, issueDate) {
     ...
     */
    async findAdditionalFees(tableId, cityId, stateId, businessPartnerId, recipientDocument) {
        try {
            ...
            for (const fee of fees) {
                if (fee.exception_group_id) {
                    ...
                    if (memberError || !memberExists) continue;
                } else {
     ...
     */
    async findTariffByCity(tableId, cityName, stateAbbr) {
        ...
        const city = cities.find(c => this.normalizeString(c.nome) === normalizedCityName);
        if (!city)
            return null;
     ...
     */
    calculateAdditionalFeeValue(fee, fretePeso, freteValor, valorMercadoria, valorCTe) {
        let calculated = 0;
     ...
     */
    performCalculation(tariff, invoiceData, cte, additionalFees = []) {
        ...
        if (!semTaxas && additionalFees.length > 0) {
            ...
            additionalFees.forEach(fee => {
                const feeValue = this.roundValue(this.calculateAdditionalFeeValue(fee, fretePeso, freteValor, invoiceData.value, valorCTeParaCalculo));
                switch (fee.fee_type) {
                    case 'TDA':
                        tda += feeValue;
                        break;
        ...
        const resultDump = {
            ...
            icmsValor,
            valorTotal,
            tariff_codigo: tariff.codigo
        };
        console.log(`[FreightCalculator Edge] Resultados para CT-e:`, resultDump);
        return {
            fretePeso,
            freteValor,
            gris,
     ...
     */
    findWeightRange(details, weight) {
     ...
     */
    calculateFretePeso(weightRange, weight, tariff) {
        if (!weightRange)
            return tariff.frete_peso_minimo || 0;
     ...
     */
    calculateFreteValor(weightRange, value, tariff) {
        if (!weightRange)
            return tariff.frete_valor_minimo || 0;
     ...
     * Calcula o pedágio
     */
    calculatePedagio(weight, tariff) {
     ...
     */
    calculateOutrosValores(baseCalculo, tariff) {
     ...
     */
    async saveCostsToCTe(cteId, calculation) {
