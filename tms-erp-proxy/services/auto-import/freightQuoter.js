import { supabase } from './freightCalculator.js';
import { freightCostCalculator } from './freightCalculator.js';
export async function calculateBestFreight(supabase, cityName, stateAbbr, weight, volumeQty, cargoValue, cubicMeters = 0) {
    try {
        ...
        for (const rateData of ratesData) {
            try {
                ...
                const calculation = await freightCostCalculator.performCalculation(tariff, {
                    weight: weight,
                    value: cargoValue,
                    volume: volumeQty,
                    m3: cubicMeters
...
export async function calculateCteFreight(supabase, cteId) {
    try {
