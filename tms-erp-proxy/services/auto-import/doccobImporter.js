import { parseDoccob } from './doccobParser.js';
export const processDoccobExtracted = async (supabaseClient, content, estab, detailsList) => {
    try {
        ...
        for (const parsedBill of parsedBills) {
            try {
                ...
                if (parsedBill.ctes_data && parsedBill.ctes_data.length > 0) {
                    ...
                    const ctesMap = new Map((matchedCtes || []).map((cte) => [cte.number, cte.id]));
